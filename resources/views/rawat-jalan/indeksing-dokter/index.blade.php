...
@section('content')
    <div class="max-w-full mx-auto bg-gray-100 p-4 min-h-screen">
        <div class="bg-gray-100 p-4 rounded-lg">
            ...
            <form action="{{ route('indeksing-dokter.index') }}" id="form" method="GET">
                @csrf
                    ...
                    <div>
                        ...
                        <select name="id_dokter" required class="border border-gray-300 p-2 rounded w-full bg-white">
                            ...
                            @foreach ($dokter as $dokters)
                                <option value="{{ $dokters->id }}"
                                    {{ request('id_dokter') == $dokters->id ? 'selected' : '' }}>
                                    {{ $dokters->nama }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <tbody>
                        @forelse ($data as $item)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="p-3 border-b">{{ $item->dokter }}</td>
